...
约束参考系 $frame=(A^C)^\top$，其中 $A^C$ 为接触坐标系（x 轴表示 geom0 到 geom1 方向的法向量），`frame` 为其转置。

无摩擦接触约束方程为（法向标量约束，仅取接触系 x 轴分量）：
$$
\begin{aligned}
n^\top (p_1 - p_0) &> -dist \\
n^\top (p_1 - p_0) + dist &> 0
\end{aligned}
$$
其中 $n$ 为接触法向（$A^C$ 的第 1 列，即 `frame` 的第 1 行），$(p_1-p_0)$ 为 body1 上接触点相对 body0 上接触点的向量（全局系）。上式两行等价。
